...
ccstatusline's **Custom Command** widget executes arbitrary shell commands — this is by design and documented behavior, not a vulnerability. Users configure these commands themselves in their `settings.json`. The commands run with the same permissions as the user's shell.
...
- CI runs with minimal permissions (`contents: read`)
